...
| `ReleaseMemory` | `cache:prefetch` / `cache:all` / `cache:volatile` / `None` | 清理 DiPECS-owned file cache，或释放 DiPECS-owned volatile memory cache。 |
| `PreWarmProcess` | `own:*` / `own:volatile-cache:<MB>` / `pkg:*` / `notif:*` | `own:*` 预热自身资源；`own:volatile-cache:<MB>` seed 可丢弃内存缓存；`pkg:*` / `notif:*` 发布用户可见提示。 |
