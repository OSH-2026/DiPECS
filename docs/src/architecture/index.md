...
DiPECS 是一个面向 Android 平台的本地优先 AIOS 原型。它把 Android 本地信号采集、
窗口级上下文构造、决策路由、策略审查和授权动作执行拆成独立边界，避免原始 Android
事件直接进入模型或动作执行器。

架构目标不是只证明“有隐私保护/有审计”，而是把预测结果转化为可测量的 Android
资源动作收益：例如启动预热、文件预取和 app-owned 内存释放。隐私脱敏、能力检查、
动作生命周期和审计记录是这些动作进入设备前必须通过的操作系统式控制面。
