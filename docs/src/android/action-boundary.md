...
| `ReleaseMemory` | `cache:prefetch` / `cache:all` / `None` | `CacheTrimmer`（只清自身 cache） | `cache:all` 可清所有应用缓存；`pkg:*` 可清指定包；`page` 可触发全局 `drop_caches` |
| `PreWarmProcess` | `own:*` / `pkg:*` / `notif:*` | `SystemPrewarmActivity` / 用户可见提示 | `pkg:*` / `notif:*` 可启动目标 launcher Activity 并立即 finish |
