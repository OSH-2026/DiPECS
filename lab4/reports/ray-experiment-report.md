...
| :--- | ---: | ---: | ---: |
| s1 (threads=8) | 10 | 4898 | 5024 |
| s2 (threads=4) | 10 | 5750 | 6602 |

**round_robin：**

| 节点 | 请求数 | 平均延迟 (ms) | P95 延迟 (ms) |
| :--- | ---: | ---: | ---: |
| s1 (threads=8) | 10 | 4841 | 5063 |
| s2 (threads=4) | 10 | 5780 | 6821 |

**latency_based：**

| 节点 | 请求数 | 平均延迟 (ms) | P95 延迟 (ms) |
| :--- | ---: | ---: | ---: |
| s1 (threads=8) | 11 | 4894 | 5170 |
| s2 (threads=4) | 9 | 6270 | 6831 |
...
本轮 **fixed_partition 比 serial 总耗时低约 10.1%**，是四种模式中吞吐最高的策略。
`round_robin` 与 `fixed_partition` 的总耗时非常接近（57.82s vs 57.61s），说明在
