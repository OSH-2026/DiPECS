...
当前已完成单机冒烟、线程数、batch、上下文、`mmap` 和温度对比。RPC 与 Ceph
依赖第二台机器和存储集群，尚未产生数据，不使用本机模拟结果代替。
...
| RPC 开销 | 单机与双机 RPC 的同任务差值 | 待双机环境完成后测量 |
...
| 模式 | 平均耗时 | tokens/s | 成功数 | 状态 |
| :--- | ---: | ---: | ---: | :--- |
| single | 待统一任务补测 | 待补测 | 待补测 | 本机链路已可用 |
| rpc | 待补测 | 待补测 | 待补测 | 需要远端 `rpc-server` |

正式对比必须固定模型、prompt、生成长度、线程参数和重复次数，并记录网络类型、
RTT、带宽及两台机器硬件。RPC 不要求快于单机，重点是解释序列化、网络传输、
远端排队、同步等待和异构节点造成的额外成本。
