...
When `aios-action` forwards approved actions directly, it sends an execute
envelope with all of the following:

- `message_type: "execute"`.
- `issued_at_ms` and `expires_at_ms`, with a maximum accepted TTL of 5 minutes.
- `action`, a string containing the serialized `AuthorizedAction`.
- `auth.hmac_sha256`, generated over the freshness window and length-prefixed
  action JSON.

Ping payloads are accepted with token auth only and never dispatch actions. The
server still rejects stale, unsigned, malformed, or non-dispatched execute
envelopes with a JSON status response.
...
cleaned after 24 hours. Socket-dispatched actions from Rust must use the
fresh execute envelope and HMAC generated by the `aios-action` bridge.
