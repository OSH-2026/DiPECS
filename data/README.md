...
| `evaluation/next-app/academic-next-app-baselines.json` | next-app 学术 baseline 对照表，区分 direct / contextual_only / excluded_unclear 可比性 | 手工维护，`academic_next_app_baselines_test` 校验 |
...
| `evaluation/cloud/` | 云端 LLM 延迟 / 场景 / 准确率基准 | `aios-agent` cloud-llm live-API benches / `tools/evaluate/evaluate_cloud_accuracy.py` |
...
`evaluation/cloud/` 子目录下还包含 Cloud LLM 准确率评估相关的数据文件：

| 文件 | 说明 |
|------|------|
| `evaluation/cloud/cloud-llm-accuracy-cases.json` | 手工编写的30条 seed 测试用例（schema v1），覆盖多种 persona 和场景 |
| `evaluation/cloud/cloud-llm-accuracy-cases.battery.json` | 从电池/应用使用数据生成的用例（300 条） |
| `evaluation/cloud/cloud-llm-accuracy-cases.lsapp.json` | 从 LSApp 生成的用例（1000 条，action-intent 模式） |
| `evaluation/cloud/cloud-llm-accuracy-cases.lsapp-next-app.json` | 从 LSApp 生成的 next-app 预测用例（100 条） |
| `evaluation/cloud/cloud-llm-accuracy-cases.lsapp-action-intent.json` | 从 LSApp 生成的 action-intent 用例（100 条） |
| `evaluation/cloud/cloud-accuracy-topk-*.json` | Python 评估器（`tools/evaluate/evaluate_cloud_accuracy.py`）的运行结果，schema v2 |
